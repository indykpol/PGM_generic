...
for (i in beg:end){
	...
	for (fold in 0:13) {
		...
		if (fold == 13) index_G2_fold <- c(index_G2_fold,(1:length(G2_template))[-(1:((fold+1)*4))])
		...
		for (current_sample in 1:length(G1)) {
			# expression
			read_count <- as.numeric(trunc(counts[workingList_BRCA[i],G1[current_sample]]))
		...
		for (current_sample in 1:length(G2)) {
			# expression
			read_count <- as.numeric(trunc(counts[workingList_BRCA[i],G2[current_sample]]))
		...
		if (sd(Ds) != 0 & D > 0.1) Zs[fold+1] <- (D - mean(Ds)) / sd(Ds) else Zs[fold+1] <- -6
		###########################################################################
		...
		for (current_sample in 1:length(G1_eval)) {
			# expression
			read_count <- as.numeric(trunc(counts[workingList_BRCA[i],G1_eval[current_sample]]))
		...
		for (current_sample in 1:length(G2_eval)) {
			# expression
			read_count <- as.numeric(trunc(counts[workingList_BRCA[i],G2_eval[current_sample]]))
	...
	} # done here with the 14-fold x-val
	###########################################################################################
