...
opt <- list(data = "data/RData/data_BRCA_progressing_lazyReady.R_CACHE", results="data/latestResults/top_100_each_seed.RData",  name = "PINCAGE_EBADIMEX_50randomSamples_BRCA_progressing_LS", folds = 50, n_top = 100, parallel=8)
...
for (cv_run in 1:50) {
	...
	if (opt$parallel > 1) {
		out <- mclapply(1:length(current_genes), mc.cores = opt$parallel, FUN = function(gene) {
			...
			genedata <- as.data.frame(read_genedata(cache = opt$data, genename = current_genes[gene])) %>% 
				dplyr::select(starts_with("pr_"), starts_with("gb_"), matches("read_count"), matches("lib_size"))
			...
			# expression
			temp_fractions <- matrix(ncol=2)
			colnames(temp_fractions) <- c("fraction","density")
			for (j in 1:nrow(genedata)) {
				...
				current <- genedata[c(G2, G1)[j], "lib_size"]
				temp_fractions <- rbind(temp_fractions, cbind(X / current, dpois(X, lambda = lambda) * current))
			}
			temp_fractions <- as.data.frame(temp_fractions[-1,],)
			temp_fractions <- temp_fractions[order(temp_fractions$fraction),]
			temp_fractions[,3] <- cumsum(temp_fractions[,2])
			temp_fractions[,3] <- temp_fractions[,3]/max(temp_fractions[,3])
			breaks <- NULL
			noBreaks <- res_expr-1
			for (j in 1:noBreaks) { breaks <- c (breaks, temp_fractions[which(temp_fractions[,3] >= j*(1/(1+noBreaks))),1][1])}
			...
			for (current_sample in 1:length(G1)) {
				# expression
				read_count <- as.numeric(trunc(genedata[G1[current_sample], "read_count"])) # adding a pseudo-count to avoid problems with 0-count data
				lambdas <- breaksEXPRESSION * genedata[G1[current_sample], "lib_size"]
			...
			for (current_sample in 1:length(G2)) {
				# expression
				read_count <- as.numeric(trunc(genedata[G2[current_sample], "read_count"])) + 1 # adding a pseudo-count to avoid problems with 0-count data
				lambdas <- breaksEXPRESSION * genedata[G2[current_sample], "lib_size"]
				frequencies_expr <- rep(0,length(breaksEXPRESSION)-1)
				for (freq in 1:res_expr) {
					frequencies_expr[freq] <- integrate(integrand_e, lower = lambdas[freq], upper = lambdas[freq+1], read_count,stop.on.error=FALSE)[1]
				}
